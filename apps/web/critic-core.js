...
export class CriticContractError extends Error {
  constructor(message, { cause = null } = {}) {
    ...
    this.code = 'CRITIC_CONTRACT_ERROR';
...
function normalizeText(value, { field, minimum = 1, maximum, providerLabel = 'Temporary critic' }) {
  if (typeof value !== 'string') {
    throw new CriticContractError(`${providerLabel} response field ${field} must be text.`);
  }
  const normalized = value.trim().replace(/\s+/g, ' ');
  if (normalized.length < minimum || normalized.length > maximum) {
    throw new CriticContractError(`${providerLabel} response field ${field} failed length validation.`);
  }
  return normalized;
}
...
export function buildCriticContext(result) {
  ...
  const inputBytes = textEncoder.encode(JSON.stringify(context)).byteLength;
  if (inputBytes > MAX_CRITIC_CONTEXT_BYTES) {
    throw new CriticContractError('Compiled temporary-critic context exceeded the allowed size.');
  }
  return { context, inputBytes };
}
...
export function parseCriticReview(content, { providerLabel = 'Temporary critic' } = {}) {
  ...
  if (typeof content !== 'string') throw new CriticContractError(`${label} response did not contain text.`);
  const withoutFence = content.trim().replace(/^```(?:json)?\s*/i, '').replace(/\s*```$/, '');
  const start = withoutFence.indexOf('{');
  const end = withoutFence.lastIndexOf('}');
  if (start < 0 || end <= start) throw new CriticContractError(`${label} response did not contain a JSON object.`);

  let parsed;
  try {
    parsed = JSON.parse(withoutFence.slice(start, end + 1));
  } catch (cause) {
    throw new CriticContractError(`${label} returned malformed JSON.`, { cause });
  }
  if (!isRecord(parsed)) throw new CriticContractError(`${label} review must be an object.`);

  const allowedKeys = new Set(['verdict', 'summary', 'confidence', 'findings']);
  if (Object.keys(parsed).some((key) => !allowedKeys.has(key))) {
    throw new CriticContractError(`${label} review contained unsupported fields.`);
  }
  if (!['pass', 'revise'].includes(parsed.verdict)) {
    throw new CriticContractError(`${label} review verdict must be pass or revise.`);
  }
  if (!Array.isArray(parsed.findings) || parsed.findings.length > MAX_CRITIC_FINDINGS) {
    throw new CriticContractError(`${label} review findings exceeded the allowed shape.`);
  }

  const findings = parsed.findings.map((finding, index) => {
    if (!isRecord(finding)) throw new CriticContractError(`${label} finding ${index + 1} must be an object.`);
    ...
    if (Object.keys(finding).some((key) => !findingKeys.has(key))) {
      throw new CriticContractError(`${label} finding ${index + 1} contained unsupported fields.`);
    }
    if (!ALLOWED_CATEGORIES.has(finding.category)) {
      throw new CriticContractError(`${label} finding ${index + 1} used an unsupported category.`);
    }
    if (!ALLOWED_SEVERITIES.has(finding.severity)) {
      throw new CriticContractError(`${label} finding ${index + 1} used an unsupported severity.`);
    }
    return {
