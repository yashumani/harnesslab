...
const FACTORS = Object.freeze([
  {
    ...
    weight: 14,
    patterns: [/\b(interpret|reason|research|compare|investigate|diagnose|recommend|ambiguous|contradictory|root[- ]cause)\b/i],
  ...
  },
  {
    ...
    weight: 13,
    patterns: [/\b(fixed|exact|predefined|step[- ]by[- ]step|template|schema|rules?|classification|extract|convert|format|three action items|single response)\b/i],
  ...
  },
  {
    ...
    weight: 14,
    patterns: [/\b(retry|recover|iterate|plan|debug|monitor|until|replan|investigate|root[- ]cause|test candidate|failure|fallback)\b/i],
  ...
  },
  {
    ...
    weight: 12,
    patterns: [/\b(api|database|sql|warehouse|github|gitlab|drive|slack|email|calendar|filesystem|snowflake|bigquery|jira|linear|salesforce|mcp|tool|repository)\b/i],
  ...
  },
  {
    ...
    weight: 13,
    patterns: [/\b(parallel|independent workstreams?|independent specialists?|multiple specialists?|separate analyses|concurrently|in parallel|fan[- ]out|map[- ]reduce)\b/i],
  ...
  },
  {
    ...
    weight: 14,
    patterns: [/\b(a2a|agent[- ]to[- ]agent|remote agent|partner agent|external agent|independent agent service|separately operated agent)\b/i],
  ...
  },
  {
    ...
    weight: 9,
    patterns: [/\b(delete|production|deploy|payment|purchase|transfer|credential|secret|customer data|medical|health|write|modify|send|publish|merge)\b/i],
  ...
  },
  {
    ...
    weight: 7,
    patterns: [/\b(evidence|cite|citation|validate|verify|audit|trace|confidence|evaluation|score|acceptance criteria|trusted sources?)\b/i],
  ...
  },
  {
    ...
    weight: 4,
    patterns: [/\b(document|knowledge|policy|manual|runbook|historical|web search|retrieval|rag|source material|trusted sources?)\b/i],
...
function protocolGuidance(factors, selected) {
  ...
  const typedDecision = tools.score === 0 ? 'recommended' : tools.score < 75 ? 'recommended' : 'optional';
  const mcpDecision = tools.score >= 75 ? 'recommended' : 'not-needed';
  ...
  return [
    ...
    },
    {
      ...
      rationale: mcpDecision === 'recommended' ? 'Multiple named systems or tools justify a consistent discovery and permission boundary.' : 'A separate MCP server is unnecessary until multiple reusable capabilities need standardization.',
      evidence: tools.evidence
    },
    {
      id: 'retrieval',
...
export function analyzeAgentDecision(rawRequirement, requirementAnalysis = null) {
  ...
  const decision = {
    ...
    readiness: {
      ...
      contradictionCount: Array.isArray(requirementAnalysis?.contradictions) ? requirementAnalysis.contradictions.length : 0,
      executionReady: requirementAnalysis?.status === 'ready' && !requirementAnalysis?.contradictions?.length
    }
  };
...
export function validateAgentDecision(value) {
  ...
  if (!Array.isArray(value.factors) || value.factors.length !== MAX_FACTORS) {
    ...
    value.factors.forEach((factor, index) => {
      ...
      if (!isRecord(factor) || factor.id !== definition.id || factor.label !== definition.label || factor.weight !== definition.weight || !['absent', 'present', 'strong'].includes(factor.state) || !Number.isFinite(factor.score) || factor.score < 0 || factor.score > 100 || !Array.isArray(factor.evidence) || factor.evidence.length > MAX_EVIDENCE_ITEMS || factor.evidence.some((item) => !isText(item)) || !isText(factor.effect)) {
        errors.push(`agentDecision.factors[${index}] is invalid.`);
      }
      if (factor?.state === 'absent' && factor?.evidence?.length) errors.push(`agentDecision.factors[${index}] cannot contain evidence when absent.`);
    });
  }
  if (!Array.isArray(value.alternatives) || value.alternatives.length !== MODES.length) {
    ...
    if (value.alternatives.filter((item) => item.status === 'selected').length !== 1) errors.push('agentDecision.alternatives must contain one selected mode.');
  }
